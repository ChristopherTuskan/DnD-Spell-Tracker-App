...
<title>Edit Spell List</title>
</head>
<body>
	<div class="col-12 mx-auto bg-dark p-3 text-white rounded m-3">
		<div class="d-flex">
			<h1 class="m-3 text-danger">Spells for ${dndChar.getName()}</h1>
		</div>
		<form action="/spells/update" method="post">
						...
						<div class="d-flex flex-column m-2">
							<c:forEach var="spell" items="${classSpellList}">
								<c:if test="${spell.getLevel() == 0}">
									...
									<c:choose>
										<c:when test="${dndChar.charHasSpell(spell)}">
											<div>
												<input name="spells[]" type="checkbox" value="${spell.getName()}" checked/>
			...
			<c:if test="${classSpellList.isEmpty() == false}">
				<input class="display-6 m-2 bg-warning" type="submit" value="Update"/>
			</c:if>
		</form>
	</div>
</body>
